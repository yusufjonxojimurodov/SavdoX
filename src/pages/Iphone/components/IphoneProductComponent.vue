...
<template>
  <div class="container">
    <template v-if="iphoneFilterproductsStore.iphoneProducts.length > 0">
      <swiper :modules="[Mousewheel]" slides-per-view="auto" :space-between="20" :mousewheel="{ forceToAxis: true }"
        :grab-cursor="true" class="!mt-6">
        <swiper-slide v-for="product in iphoneFilterproductsStore.iphoneProducts" :key="product._id" class="!w-[300px]">
          <div class="product transition duration-500 bg-[#1E1E1E]
            ...
            <div class="flex flex-col w-full gap-2 sm:gap-3">
              ...
              <p class="text-[14px] sm:text-[16px] md:text-[18px] text-[#FFD700] w-[70px] rounded-[10px] font-semibold">
                {{ product.price }}$
              </p>
