    ...
    <div class="container">
      <template v-if="iphoneFilterproductsStore.iphoneProducts.length > 0">
        <div class="grid gap-4 sm:gap-6 !mt-6 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <div v-for="product in iphoneFilterproductsStore.iphoneProducts" :key="product._id"
            ...
            <img :src="product.image" alt="Mahsulot rasmi" class="w-full !h-[240px] object-contain rounded-2xl" />
            ...
            <div class="!flex flex-col !gap-2 sm:!gap-3 !flex-1">
              ...
              <div class="flex justify-between items-center w-full">
                <div class="flex items-center gap-1 sm:gap-2 w-full">
                  ...
                  <p class="flex justify-center items-center gap-2 text-red-500 text-[12px] sm:text-[14px] font-medium">
                    <icon-sad-component class="fill-red-500" /> {{ product.rating.unhappy }}%
                  </p>
                </div>
                <p class="text-[#888] text-[12px] sm:text-[14px] font-medium">{{ product.model }}</p>
              </div>
            </div>
