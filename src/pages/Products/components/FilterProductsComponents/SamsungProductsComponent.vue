...
<template>
    <div class="container">
        <template v-if="products.length > 0">
          ...
          grid 
          gap-4 sm:gap-6 !mt-6
          grid-cols-2        /* Telefonlarda 2 ustun */
          md:flex md:flex-wrap md:justify-start md:items-center
        ">
                <div class="
            product transition duration-500 bg-[#1E1E1E]
            w-full md:w-[300px]
            h-auto md:h-[430px]
          ...
          " v-for="product in samsungProductsStore.samsungProducts" :key="product._id">
                    <img :src="axiosInstance.defaults.baseURL + product.image" alt="Mahsulot rasmi"
                        class="w-full h-[120px] sm:h-[150px] md:h-[180px] rounded-2xl transition duration-500 object-cover" />
